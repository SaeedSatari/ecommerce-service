...
class ProductCategoryServiceTest {
  ...
  @Test
  @DisplayName(
    "fetchProductCategories when find all the product categories then should returns list of the product categories"
  )
  void fetchProductCategories_whenFindAllTheProductCategories_thenShouldReturnsListOfTheProductCategories() {
    ...
    assertTrue(
      anyCategories.containsAll(persistedCategories) &&
      persistedCategories.containsAll(anyCategories)
    );
  }

  @Test
  @DisplayName(
    "fetchProductCategories when can not find product categories then should throws EntityNotFoundException"
  )
  ...
  @Test
  @DisplayName(
    "fetchProductCategory when find a product category with given id then should returns the product category"
  )
  void fetchProductCategory_whenFindAProductCategoryWithGivenId_thenShouldReturnsTheProductCategory() {
    ...
    doReturn(Optional.of(anyProductCategory))
      .when(categoryRepository)
      .findById(anyLong());
    ProductCategory persistedProductCategory = categoryService.fetchProductCategory("1");
    assertEquals(
      anyProductCategory.getCategoryName(),
      persistedProductCategory.getCategoryName()
    );
  }

  @Test
  @DisplayName(
    "fetchProductCategory when can not find a product category with given id then should throws EntityNotFoundException"
  )
