...
class ProductControllerIT {
  ...
  void getPagedProducts_whenRequestIsValid_thenReturnsListOfProducts() throws Exception {
    ...
    mockMvc
      ...
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.products[*].categoryName").isNotEmpty()
      )
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.products[*].unitsInStock").isNotEmpty()
      )
  ...
  @Test
  @DisplayName(
    "getProductDetails when request is valid then should returns founded product response"
  )
  void getProductDetails_whenRequestIsValid_thenShouldReturnsFoundedProductResponse()
    throws Exception {
  ...
  @Test
  @DisplayName(
    "searchByCategoryId when request is valid then returns list of the products"
  )
  void searchByCategoryId_whenRequestIsValid_thenReturnsListOfProducts()
    throws Exception {
  ...
  void searchByCategoryId_whenRequestIsInvalid_thenResponseClientError()
    throws Exception {
    mockMvc
      .perform(
        MockMvcRequestBuilders
          .get("/api/products/search/categoryIds")
          .param("id", "1")
          .contentType(MediaType.APPLICATION_JSON)
      )
      .andExpect(status().is4xxClientError());
